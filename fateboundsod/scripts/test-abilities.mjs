...
test('187 unique IDs and explicit definitions',()=>{assert.equal(cards.length,187);assert.equal(new Set(cards.map(c=>c.id)).size,187);for(const c of cards)assert.ok(Object.hasOwn(definitions,c.name),c.name);assert.equal(byName['Cryo Mage'].id,'CRTE_CYO_0008');assert.equal(byName['Ice Protector'].id,'CRTE_CYO_0006');});
test('all 27 controller starts resolve',()=>{for(const c of cards.filter(c=>c.card_type==='controller')) {const g=newMatch(c.element,'fire',c.name,'Flame Emperor',42);assert.equal(g[P].controllers[0].card.name,c.name);assert.ok(g[P].hand.length>=5);assert.ok(g[P].shards>=0);}});
