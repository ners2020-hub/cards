...
declare
 ...
 a jsonb; b jsonb; before_tokens integer; code text:='STORE_TEST_'||replace(gen_random_uuid()::text,'-','');
...
begin
 ...
 assert public.store_request(actor)->'owned'=a->'owned','Account initialization must be idempotent';
 b:=public.store_request(actor,'pack','starter',req);
 ...
 a:=public.store_request(actor,'pack','starter',req);
 assert a->'cards'=b->'cards' and (a->>'tokens')::integer=50,'Retry must not charge or reroll';
 begin perform public.store_request(actor,'pack','elite',req);raise exception 'FAIL: reused ID accepted'; exception when raise_exception then if sqlerrm like 'FAIL:%' then raise; end if;end;
 begin perform public.store_request(actor,'pack','elite',gen_random_uuid());raise exception 'FAIL: insufficient funds accepted'; exception when raise_exception then if sqlerrm like 'FAIL:%' then raise; end if;end;
 assert (public.store_request(actor)->>'tokens')::integer=50,'Rejected purchase must not charge';
 begin perform public.store_request(actor,'pack','unknown',gen_random_uuid());raise exception 'FAIL: unknown pack accepted'; exception when raise_exception then if sqlerrm like 'FAIL:%' then raise; end if;end;
 begin perform public.store_request(actor,'unlock','blood',gen_random_uuid());raise exception 'FAIL: unearned unlock accepted'; exception when raise_exception then if sqlerrm like 'FAIL:%' then raise; end if;end;
 ...
 a:=public.store_request(other_actor);
 assert (a->>'tokens')::integer=777 and (a->'owned'->>newid)::integer=3,'Migration must preserve balance and map by identity';
 a:=public.store_request(other_actor,'unlock','blood',gen_random_uuid());
 assert (a->>'tokens')::integer=277 and a->'unlocked' ? 'blood','Qualified unlock must charge 500 once';
 begin perform public.store_request(other_actor,'unlock','blood',gen_random_uuid());raise exception 'FAIL: duplicate unlock accepted'; exception when raise_exception then if sqlerrm like 'FAIL:%' then raise; end if;end;
 update public.store_accounts set tokens=3000 where user_id=actor;
 foreach code in array array['premium','elite'] loop
  a:=public.store_request(actor,'pack',code,gen_random_uuid());
  assert jsonb_array_length(a->'cards')=5,'Every pack must give exactly five cards';
 end loop;
 code:='STORE_TEST_'||replace(gen_random_uuid()::text,'-','');
 ...
 before_tokens:=(public.store_request(actor)->>'tokens')::integer;
 req:=gen_random_uuid();a:=public.store_request(actor,'promo',code,req);
 assert (a->>'tokens')::integer=before_tokens+200 and jsonb_array_length(a->'cards')=2,'Promo must grant tokens and canonical cards';
 b:=public.store_request(actor,'promo',code,req);
 assert a->'tokens'=b->'tokens','Promo retry must not duplicate rewards';
 begin perform public.store_request(actor,'promo',code,gen_random_uuid());raise exception 'FAIL: repeated promo accepted'; exception when raise_exception then if sqlerrm like 'FAIL:%' then raise; end if;end;
 begin perform public.store_request(other_actor,'promo',code,gen_random_uuid());raise exception 'FAIL: exhausted promo accepted'; exception when raise_exception then if sqlerrm like 'FAIL:%' then raise; end if;end;
 assert not has_function_privilege('authenticated','public.store_request(uuid,text,text,uuid)','EXECUTE'),'Clients must not forge actors';
 ...
 assert not has_function_privilege('anon','public.store_request(uuid,text,text,uuid)','EXECUTE'),'Anonymous calls must fail';
