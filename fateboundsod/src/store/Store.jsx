...
function Store() {
 ...
 const [account,setAccount]=useState(null),[notice,setNotice]=useState(''),[busy,setBusy]=useState(false),[tab,setTab]=useState('packs');
 ...
 async function buy(op,item,retry=null){await run(async()=>{
  ...
  try {const next=await request(body);setAccount(next);setPending(null);sessionStorage.removeItem(pendingKey);
   if(next.cards.length)setReveal({cards:next.cards,title:op==='promo'?'Your rewards':'Your new cards'});
   setNotice(op==='unlock'?'Deck unlocked.':op==='promo'?`Promo redeemed. ${Math.max(0,-next.cost)} tokens added.`:'Five cards added to your collection.');
 ...
 return <div className="fate-app store-app">
  <header className="fate-header"><a className="brand" href="/"><Diamond size={28}/><span>FATEBOUND<small>SHARDS OF DOMINION</small></span></a><nav className="header-tools" aria-label="Game navigation"><a href="/">Arena</a><a href="/multiplayer">Multiplayer</a>{user&&<button disabled={busy||!!pending} onClick={()=>run(signOut)}>Sign out</button>}</nav></header>
  <main className="store-main"><a className="store-back" href="/"><ArrowLeft size={15}/> Back to the arena</a>
   <section className="store-intro"><div><p className="eyebrow">THE CARD STORE · BALANCE 0.3</p><h1>A little chance.<br/><em>A new possibility.</em></h1><p>Open a pack. Discover a favorite. Build your collection across the nine dominions.</p></div><div className="store-wallet"><Sparkles size={24}/><small>YOUR TOKENS</small><strong>{account?account.tokens.toLocaleString():'—'}</strong><span>{user?`${ownedCount} cards collected`:'100 starting tokens for new players'}</span></div></section>
   ...
   <nav className="store-tabs" aria-label="Store sections">{[['packs','Packs'],['collection','My collection'],['decks','Deck unlocks']].map(([id,label])=><button key={id} aria-current={tab===id?'page':undefined} onClick={()=>setTab(id)}>{id==='collection'&&<Layers size={16}/>}{label}</button>)}</nav>
