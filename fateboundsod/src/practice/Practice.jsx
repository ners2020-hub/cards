...
import { DEFAULT_RULES, hasElement, elementLabel } from './catalog.js';
import './practice.css';
...
function Card({ card, instance, onClick, selected, ready, onInspect, onAbility, cost }) {
  ...
  const statuses = Object.entries(instance?.statuses || {}).map(([name, data]) => describeStatus(name, data));
  const inspectCard = () => onInspect({ ...(cards.find(c => c.id === card.id) || card), unitUid: instance?.uid });
  return <motion.div data-unit={instance?.uid} layout={!reduced} initial={reduced ? false : { opacity: 0, y: 25, scale: .9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0 }} className={`duel-card ${selected ? 'selected' : ''} ${ready ? 'ready' : ''}`} style={{ '--element': colors[card.element] || '#d1c6ad' }}>
    <button className="card-face" onClick={onClick} aria-label={`${card.name}, ${cost ?? card.cost} shards${isUnit ? `, ${instance?.currentAP ?? card.ap} attack, ${instance?.currentCH ?? card.ch} health` : `, ${card.card_type}`}`}>
      ...
      <span className="card-caption"><strong>{card.name}</strong><span>{isUnit ? <b>⚔ {instance?.currentAP ?? card.ap}</b> : <b>✦</b>}<small>{card.card_type === 'creature' ? elementLabel(card) : card.card_type}</small>{isUnit && <b>♥ {instance?.currentCH ?? card.ch}</b>}</span></span>
    </button>
    <button className="card-info" aria-label={`Inspect ${card.name}`} onClick={inspectCard}>i</button>
    {onAbility && <button className="ability-button" onClick={onAbility} aria-label={`Abilities of ${card.name}`}><Sparkles size={13} /></button>}
    {statuses.length > 0 && <button className="unit-statuses status-explainer" onClick={inspectCard} aria-label={`Explain effects on ${card.name}`} title={statuses.map(s => `${s.name}: ${s.description} ${s.duration}`).join('\n')}>{statuses.slice(0, 2).map(s => s.name).join(' · ')}{statuses.length > 2 ? ` +${statuses.length - 2}` : ''} ⓘ</button>}
...
export default function Practice({ remote = null }) {
  ...
  function unitClick(unit) {
    ...
    else setInspect({ ...unit.card, unitUid: unit.uid });
  }
  ...
  const inspectedUnit = inspect?.unitUid && game && ['playerState', 'opponentState'].flatMap(side => ['controllers', 'creatures', 'artifacts', 'spells'].flatMap(zone => game[side][zone].filter(Boolean))).find(unit => unit.uid === inspect.unitUid);
  const activeEffects = Object.entries(inspectedUnit?.statuses || {}).map(([name, data]) => describeStatus(name, data));
  const hero = cards.find(c => c.name === controller) || cards.find(c => c.element === element && c.card_type === 'controller');
  const shown = cards.filter(c => (filter === 'all' || hasElement(c, filter)) && `${c.name} ${c.description} ${c.card_type}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <div ref={playback.root} className="fate-app" style={{ '--accent': colors[element] }}>
    ...
    {!game ? <main className="lobby"><section className="lobby-copy"><div className="eyebrow">THE ARENA AWAITS</div><h1>Choose your fate.<br /><em>Claim dominion.</em></h1><p>Command your creatures. Cast your spells. Equip powerful artifacts and claim the battlefield.</p>
      <div className="mode-summary"><Swords /><div><strong>A duel of elements</strong><span>{cards.length} card designs · Deathbound now available</span></div></div>
      ...
      <p>{hero.archetype === 'deathbound' ? 'The Deathbound: 30-card Shadow/Blood Undead deck. Your selected Controller is the only Controller; Risen are created by effects.' : 'For the Deathbound deck, choose Blood → Veyra or Shadow → Mordrath.'}</p>
      ...
      <button className="primary enter" disabled={startingGame} onClick={begin}>{startingGame ? 'Opening arena…' : 'Enter the arena'} <ArrowRight size={20} /></button><p role="status">{startError}</p><p className="scope-note">Sign in before starting to earn 10 tokens per AI win, 3 per loss, and quest progress. Guest games are practice only.</p>
    ...
    <main className="arena-layout"><section className="arena ability-arena"><div className="arena-top"><button onClick={leave}><ArrowLeft size={16} /> Leave duel</button><span>THE SHATTERED SANCTUM</span>{remote ? <button disabled={remote.busy || !!winner} onClick={() => remote.act({ type: "concede" })}>Concede</button> : <button onClick={begin}>Restart</button>}</div>
      {['opponentState', 'playerState'].map(side => { const state = game[side]; const mine = side === 'playerState'; return <section className={`army ${mine ? 'friendly' : 'enemy'}`} key={side}>
        ...
        <div className="support-row"><span>ARTIFACTS</span>{state.artifacts.map((unit, i) => unit ? <button key={unit.uid} className="support-card" onClick={() => setInspect({ ...unit.card, unitUid: unit.uid })}><span>◆ {unit.card.name}</span><small>{unit.target ? [...state.creatures, ...state.controllers].find(u => u?.uid === unit.target)?.card.name || 'Marked' : 'Field artifact'}</small></button> : <span className="support-empty" key={i}>◇</span>)}</div>
        <div className="support-row"><span>SPELLS</span>{state.spells.map((unit, i) => unit ? <button key={unit.uid} className="support-card spell" onClick={() => setInspect({ ...unit.card, unitUid: unit.uid })}><span>✦ {unit.card.name}</span><small>Persistent</small></button> : <span className="support-empty" key={i}>·</span>)}{state.reaction && <span className="reaction-label">↩ {state.reaction.name} ready</span>}</div>
